...
CTexture* Model::LoadTexturesForAssimp(const char* pathforfile, std::string directorys)
{
	...
	temptex->LoadFreeImage(this->DevClass, filenames.c_str());
...
Model::Model(const char* path, DirectXDevice* devc)
{
	this->DevClass = devc;
	this->loadModel(path);
	this->path_ = path;
...
void Model::loadModel(std::string path)
{
	...
	const aiScene* scene = importer.ReadFile(path, aiProcess_GenSmoothNormals | aiProcess_CalcTangentSpace | aiProcess_Triangulate  | aiProcess_SortByPType);
	// Check for errors
	...
	std::string successstring = "Loaded model " + path + "Mesh count: " + std::to_string(scene->mNumMeshes);
	PRINT(successstring);
...
Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	if (mesh->mMaterialIndex >= 0)
	{
		...
		if (AI_SUCCESS == aiGetMaterialColor(material, AI_MATKEY_COLOR_DIFFUSE, &kd))
		{
			mp.kd = XMFLOAT3(kd.r, kd.g, kd.b);
		}
		if (AI_SUCCESS == aiGetMaterialColor(material, AI_MATKEY_COLOR_SPECULAR, &ks))
		{
			mp.ks = XMFLOAT3(ks.r, ks.g, ks.b);
		}
	}

	// Return a mesh object created from the extracted mesh data
	return Mesh(avertices, aindices, atextures, DevClass, mp);
}
